...
$breadcrumbs = [ ['label' => 'Home', 'url' => '/'], ['label' => 'Assigned Tasks'] ];
                    ...
                    <?php foreach ($rows as $row): ?>
                        <tr class="border-t" style="border-color:#D7DDE5;">
                            ...
                            <td class="px-3 py-2">
                                ...
                                $bg2 = $statusColors[$s][0] ?? '#D7DDE5';
                                $fg2 = $statusColors[$s][1] ?? '#2C3E50';
                                ?>
                                <span class="px-2 py-1 rounded-full text-xs font-semibold" style="background:<?php echo $bg2; ?>;color:<?php echo $fg2; ?>;">
                                    <?php echo htmlspecialchars($row['status']); ?>
                                </span>
                            </td>
                            <td class="px-3 py-2"><?php echo htmlspecialchars($row['date']); ?></td>
                            <td class="px-3 py-2"><?php echo htmlspecialchars($row['due']); ?></td>
                            <td class="px-3 py-2 flex flex-wrap gap-2">
                                <a href="/tech/tasks/<?php echo urlencode($row['id']); ?>" class="text-sm font-semibold" style="color:#1F4E79;">Details</a>
                                <a href="/tech/tasks/<?php echo urlencode($row['id']); ?>/start" class="text-sm font-semibold" style="color:#3498DB;">Start Work</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
